...
4. In the `src/components/` folder, create `Search.jsx` that exports the `Search` component. Let it return a `form` tag with a `fieldset` tag, `legend` tag, and TWE inputs to get product ID, product name, and max price. Initialize the inputs with `useInputTWE`. This form will get search criteria without a form handler nor a submit button. In `WebClient.jsx`, render `Search` below the `DbPassword` component.
